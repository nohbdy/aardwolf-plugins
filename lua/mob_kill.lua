...
local function print_kill(name, line, wildcards)
    ...
    ColourTell("yellow", "black", " " .. wildcards.mob)
    ColourNote("grey", "black", " (" .. source .. ")")
end
...
local function create_triggers()
    for reason, text in pairs(kill_text_map) do
        ...
        -- print("Adding kill trigger for " .. reason .. ": " .. regex)

        local trigger_name = string.gsub(reason, " ", "_")
        Trigger.new(trigger_name, "KilledBy", regex, { omit_from_output = true, enabled = true, keep_evaluating = true }, print_kill)
    end
end
